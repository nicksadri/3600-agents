...
class PlayerAgent:
    """
    Epicenter V2.2: Eggs-first alpha–beta agent with supportive turd usage.

    - Uses Bayesian trapdoor tracking (TrapdoorTracker).
    - Iterative deepening alpha–beta search.
    - Root-level hard rule: never step on a discovered trapdoor if avoidable.
    - Heuristic includes:
        * Egg advantage (primary)
        * Turd usage advantage (small bonus for using turds)
        * Turd economic advantage (my turds deny their egg squares minus theirs deny mine)
        * Mobility advantage
        * Trapdoor risk at our own location

    Turds are now helpful, but cannot outweigh eggs.
    """
    ...
    def __init__(self, board: Board, time_left: Callable[[], float]):
        ...
        self.max_turds = board.game_map.MAX_TURDS  # typically 5
        ...
        # Heuristic weights (key change: eggs >> turds)
        self.w_egg_adv = 10.0       # eggs are the main score
        self.w_turd_res = 0.2       # small bonus for using turds
        self.w_turd_eco = 1.0       # modest turd economic impact
    ...
    def _turd_usage_advantage(self, board: Board, root_is_player: bool) -> float:
        """
        Reward *using* turds (slightly), not hoarding them.

        turds_placed = MAX_TURDS - turds_left

        Advantage = (my turds placed) - (opp turds placed)
        """
        _, _, my_turds_left, opp_turds_left, _, _, _, _ = \
            self._get_root_perspective_values(board, root_is_player)

        my_placed = self.max_turds - my_turds_left
        opp_placed = self.max_turds - opp_turds_left

        return float(my_placed - opp_placed)
    ...
    def _turd_economic_advantage(self, board: Board, root_is_player: bool) -> float:
        """
        Simpler turd economic advantage:

        + my turds blocking opponent egg squares
        - opponent turds blocking my egg squares

        Corner squares count as 3, normal parity squares as 1.
        No parity self-harm modeling here; we keep it simple.
        """
        (
            _my_eggs_score,
            _opp_eggs_score,
    ...
    def _evaluate(self, board: Board, root_is_player: bool) -> float:
        ...
        # 2. Turd usage – small nudge to actually spend turds
        score += self.w_turd_res * self._turd_usage_advantage(board, root_is_player)

        # 3. Turd economic advantage – helpful but secondary
